...
export class Document extends SchemaDocumentBase<Draft06Schema> {
	...
	protected isDocumentNode(node: unknown): node is Draft06Schema {
	...
	public *getEmbeddedDocuments(retrievalUrl: URL): Iterable<EmbeddedDocument> {
		const queue = new Array<readonly [string, Draft06Schema]>();
		queue.push(
			...selectors.selectSubNodes(this.documentNodePointer, this.documentNode),
		);

		let pair: readonly [string, Draft06Schema] | undefined;
	...
	protected *getNodePairs(): Iterable<readonly [string, Draft06Schema]> {
		const queue = new Array<readonly [string, Draft06Schema]>();
		queue.push(
			...selectors.selectSubNodes(this.documentNodePointer, this.documentNode),
		);
