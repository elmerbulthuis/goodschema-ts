...
export class Document extends SchemaDocumentBase<
	Draft04Schema | boolean | boolean
> {
	...
	protected isDocumentNode(node: unknown): node is Draft04Schema | boolean {
	...
	public *getEmbeddedDocuments(retrievalUrl: URL): Iterable<EmbeddedDocument> {
		const queue = new Array<
			readonly [string, Draft04Schema | boolean | boolean]
		>();
		queue.push(
			...selectors.selectSubNodes(this.documentNodePointer, this.documentNode),
		);

		let pair: readonly [string, Draft04Schema | boolean | boolean] | undefined;
	...
	protected *getNodePairs(): Iterable<
		readonly [string, Draft04Schema | boolean | boolean]
	> {
		const queue = new Array<
			readonly [string, Draft04Schema | boolean | boolean]
